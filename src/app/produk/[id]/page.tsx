...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleWhatsApp = () => {
    ...
    const text = product.whatsappText || `Halo Hunay! Saya ingin memesan ${product.nameId}`;
    window.open(`https://api.whatsapp.com/send/?phone=${WHATSAPP_NUMBER}&text=${encodeURIComponent(text)}&type=phone_number&app_absent=0`, "_blank");
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 pt-32 pb-20 text-center">
          <p className="text-gray-500 text-lg mb-4">{language === "id" ? "Produk tidak ditemukan." : "Product not found."}</p>
          <Link href="/#produk" className="text-forest-green hover:underline">← {language === "id" ? "Kembali ke Produk" : "Back to Products"}</Link>
        </div>
        <Footer />
      </div>
    );
  ...
  const name = language === "id" ? product.nameId : product.nameEn;
  const description = language === "id" ? product.descriptionId : product.descriptionEn;
              ...
              <div>
                ...
                <div className="space-y-2 text-sm text-gray-500 mb-8">
                  <div className="flex gap-2">
                    <span className="font-medium text-gray-700">{language === "id" ? "Kategori" : "Category"}:</span>
                    <span>{categoryLabel}</span>
                  </div>
                  <div className="flex gap-2">
                    <span className="font-medium text-gray-700">{language === "id" ? "Berat" : "Weight"}:</span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${product.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                      {product.isActive ? (language === "id" ? "Tersedia" : "Available") : (language === "id" ? "Habis" : "Out of stock")}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                {cartQty > 0 && (
                  <p className="text-sm text-forest-green font-medium">
                    {language === "id" ? `${cartQty} item di keranjang` : `${cartQty} item in cart`}
                  </p>
                )}
                ...
                >
                  ...
                  {added ? (language === "id" ? "Ditambahkan!" : "Added!") : t("products.addToCart")}
                </button>
                ...
                <Link href="/#produk" className="block text-center text-sm text-gray-400 hover:text-forest-green transition mt-2">
                  ← {language === "id" ? "Lihat produk lain" : "See other products"}
                </Link>
              </div>
            </div>
          </div>
